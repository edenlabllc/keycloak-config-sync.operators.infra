...
import (
	...
	"github.com/edenlabllc/keycloak-config-sync.operators.infra/pkg/secretref/mocks"
)
...
func TestPutClient_Serve(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCondition: &metav1.Condition{
				Type:   chain.ConditionClientSynced,
				Status: metav1.ConditionTrue,
				Reason: chain.ReasonClientUpdated,
			},
		},
		{
							...
							&keycloakApiAlpha.KeycloakClient{
								...
								Spec: keycloakApiAlpha.KeycloakClientSpec{
									Client: []keycloakApiAlpha.Client{
										{
											ClientId: "test-client-id",
											AuthenticationFlowBindingOverrides: &keycloakApiAlpha.AuthenticationFlowBindingOverrides{
												Browser:     "browser",
