...
func (h *ProcessPolicy) Serve(ctx context.Context, keycloakClient *DataClient, realmName string) error {
	...
	for i := 0; i < len(keycloakClient.Client.Authorization.Policies); i++ {
		log.Info("Processing policy", policyLogKey, keycloakClient.Client.Authorization.Policies[i].Name)

		var policyRepresentation *gocloak.PolicyRepresentation

		if policyRepresentation, err = h.toPolicyRepresentation(ctx, &keycloakClient.Client.Authorization.Policies[i], clientID, realmName); err != nil {
			h.setFailureCondition(ctx, keycloakClient, fmt.Sprintf("Failed to sync authorization policies: %s", err.Error()))

			return fmt.Errorf("failed to convert policy: %w", err)
		}

		existingPolicy, ok := existingPolicies[keycloakClient.Client.Authorization.Policies[i].Name]
		if ok {
			...
			log.Info("Policy updated", policyLogKey, keycloakClient.Client.Authorization.Policies[i].Name)

			delete(existingPolicies, keycloakClient.Client.Authorization.Policies[i].Name)
		...
		log.Info("Policy created", policyLogKey, keycloakClient.Client.Authorization.Policies[i].Name)
	}
...
func (h *ProcessPolicy) setFailureCondition(ctx context.Context, keycloakClient *DataClient, message string) {
	log := ctrl.LoggerFrom(ctx)

	if err := SetCondition(
		ctx, h.k8sClient, keycloakClient,
		chain.ConditionAuthorizationPoliciesSynced,
...
func (h *ProcessPolicy) setSuccessCondition(ctx context.Context, keycloakClient *DataClient, message string) {
	log := ctrl.LoggerFrom(ctx)
	...
	if err := SetCondition(
		...
		chain.ReasonAuthorizationPoliciesSynced,
		message,
	); err != nil {
