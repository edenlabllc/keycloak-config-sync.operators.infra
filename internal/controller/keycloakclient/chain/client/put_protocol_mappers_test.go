...
func TestPutProtocolMappers_Serve(t *testing.T) {
						...
						Client: []keycloakApiAlpha.Client{
							{
								...
								ProtocolMappers: &[]keycloakApiAlpha.ProtocolMapper{
									{
										Name:           "test-mapper",
				...
				keycloakClient: &keycloakApiAlpha.KeycloakClient{
					...
					Spec: keycloakApiAlpha.KeycloakClientSpec{
						...
						Client: []keycloakApiAlpha.Client{
							{
								ClientId:        "test-client-id",
								ProtocolMappers: &[]keycloakApiAlpha.ProtocolMapper{}, // empty slice
							},
						},
					},
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for index, client := range tt.args.keycloakClient.Spec.Client {
				dc := &DataClient{
					CRD:         tt.args.keycloakClient,
					Client:      client,
					ClientIndex: index,
					Namespace:   tt.args.keycloakClient.Namespace,
					ReasonScope: &ReasonScope{
						ClientID:   tt.args.keycloakClient.Status.GetClientIDByName("test-client"),
						Generation: tt.args.keycloakClient.Generation,
					},
				}

				err := el.Serve(context.Background(), dc, tt.args.realmName)
				tt.wantErr(t, err)
			}
		})
	}
