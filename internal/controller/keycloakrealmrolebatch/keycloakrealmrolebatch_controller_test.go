...
func TestReconcileKeycloakRealmRoleBatch_ReconcileDelete(t *testing.T) {
	...
	secret := corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "keycloak-secret", Namespace: ns},
...
func TestReconcileKeycloakRealmRoleBatch_Reconcile(t *testing.T) {
	...
	err = client.Get(context.Background(), types.NamespacedName{
		Namespace: ns,
		Name:      fmt.Sprintf("%s-sub-role2", batch.Name),
	}, &checkRole)
	require.NoError(t, err)
	...
	if err := client.Get(context.Background(), types.NamespacedName{
		Namespace: ns,
		Name:      fmt.Sprintf("%s-sub-role1", batch.Name),
	}, &checkRole); err == nil {
...
func TestReconcileKeycloakRealmRoleBatch_ReconcileFailure(t *testing.T) {
	...
	role := keycloakApi.KeycloakRealmRole{
		ObjectMeta: metav1.ObjectMeta{Name: "batch1-role2", Namespace: ns},
