...
func (r *Reconcile) SetupWithManager(mgr ctrl.Manager, successReconcileTimeout time.Duration) error {
	...
	err := ctrl.NewControllerManagedBy(mgr).
		For(&keycloakApiAlpha.KeycloakComponent{}, builder.WithPredicates(pred)).
		Complete(r)
	if err != nil {
...
func isSpecUpdated(e event.UpdateEvent) bool {
	oo, ok := e.ObjectOld.(*keycloakApiAlpha.KeycloakComponent)
	if !ok {
		return false
	}

	no, ok := e.ObjectNew.(*keycloakApiAlpha.KeycloakComponent)
	if !ok {
		return false
	}
...
// +kubebuilder:rbac:groups=config.idp.edenlab.io,namespace=keycloak,resources=keycloakcomponents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.idp.edenlab.io,namespace=keycloak,resources=keycloakcomponents/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=config.idp.edenlab.io,namespace=keycloak,resources=keycloakcomponents/finalizers,verbs=update
...
func (r *Reconcile) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	keycloakRealmComponent := &keycloakApiAlpha.KeycloakComponent{}
	...
	kClient, err := r.helper.CreateKeycloakClientFromConfigRef(ctx, keycloakRealmComponent)
	if err != nil {
...
func (r *Reconcile) createKeycloakComponent(
	ctx context.Context,
	component *keycloakApiAlpha.KeycloakComponent,
	kcRealmName string,
	kClient keycloak.Client,
...
func (r *Reconcile) getParentID(
	...
	if component.Spec.ParentRef.Kind == keycloakApiAlpha.KeycloakRealmKind {
		parentRealm := &keycloakApiAlpha.KeycloakRealm{}
	...
	if component.Spec.ParentRef.Kind == keycloakApiAlpha.KeycloakRealmComponentKind {
		parentComponent := &keycloakApiAlpha.KeycloakComponent{}
...
func (r *Reconcile) setComponentOwnerReference(
	...
	if component.Spec.ParentRef == nil || component.Spec.ParentRef.Kind != keycloakApiAlpha.KeycloakRealmComponentKind {
		return nil
	}

	for _, ref := range component.GetOwnerReferences() {
		if ref.Kind == keycloakApiAlpha.KeycloakRealmComponentKind {
			return nil
		}
	}
