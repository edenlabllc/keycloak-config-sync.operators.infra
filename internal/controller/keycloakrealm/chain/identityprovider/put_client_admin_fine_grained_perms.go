...
func (el *PutAdminFineGrainedPermissions) Serve(
	ctx context.Context,
	keycloakIDP *keycloakApiAlpha.IdentityProvider,
	realmName, _ string) error {
	...
	if keycloakIDP.AdminFineGrainedPermissionsEnabled && keycloakIDP.Permission != nil {
...
func (el *PutAdminFineGrainedPermissions) putKeycloakClientAdminFineGrainedPermissions(ctx context.Context, keycloakIDP *keycloakApiAlpha.IdentityProvider, realmName string) error {
	...
	managementPermissions := adapter.ManagementPermissionRepresentation{
		Enabled: &keycloakIDP.AdminFineGrainedPermissionsEnabled,
	}

	if err := el.keycloakApiClient.UpdateIDPManagementPermissions(realmName, keycloakIDP.Alias, managementPermissions); err != nil {
...
func (el *PutAdminFineGrainedPermissions) putKeycloakIDPAdminPermissionPolicies(ctx context.Context, keycloakIDP *keycloakApiAlpha.IdentityProvider, realmName string) error {
	reqLog := ctrl.LoggerFrom(ctx)
	reqLog.Info("Start put keycloak idp admin permission policies")

	idp, err := el.keycloakApiClient.GetIdentityProvider(ctx, realmName, keycloakIDP.Alias)
	if err != nil {
	...
	existingIDPPermissions, err := el.keycloakApiClient.GetIDPManagementPermissions(realmName, keycloakIDP.Alias)
	if err != nil {
	...
	for i := 0; i < len(keycloakIDP.Permission.ScopePermissions); i++ {
		name := keycloakIDP.Permission.ScopePermissions[i].Name
		...
		if permission, ok := realmManagementPermissions[permissionName]; ok {
			permission.Policies = &keycloakIDP.Permission.ScopePermissions[i].Policies
