...
func (el *PutIDP) Serve(
	...
	err := el.putKeycloakIDP(ctx, keycloakRealmIDP, realmName, namespace)
	if err != nil {
...
func (el *PutIDP) putKeycloakIDP(
	...
	keycloakIDP := createKeycloakIDPFromSpec(keycloakRealmIDP)

	if err = el.secretRef.MapConfigSecretsRefs(ctx, keycloakIDP.Config, namespace); err != nil {
		return fmt.Errorf("unable to map config secrets: %w", err)
	}

	providerExists, err := el.keycloakApiClient.IdentityProviderExists(ctx, realmName, keycloakRealmIDP.Alias)
	if err != nil {
...
func createKeycloakIDPFromSpec(spec *keycloakApiAlpha.IdentityProvider) *adapter.IdentityProvider {
