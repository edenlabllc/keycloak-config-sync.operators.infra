...
func TestPutAdminFineGrainedPermissions_Serve(t *testing.T) {
	...
	}{
		{
			...
			client: func(t *testing.T) client.Client {
				...
				return fake.NewClientBuilder().WithScheme(s).WithObjects(
					&keycloakApiAlpha.KeycloakRealm{
						ObjectMeta: metav1.ObjectMeta{
							Name:      "test-idp",
							Namespace: "default",
						},
						Spec: keycloakApiAlpha.KeycloakRealmSpec{
							IdentityProviders: []keycloakApiAlpha.IdentityProvider{
								{
									Alias:                              "test-idp",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			objRealm := &keycloakApiAlpha.KeycloakRealm{}
			require.NoError(t, tt.client(t).Get(context.Background(), tt.keycloakIDP, objRealm))

			el := NewPutAdminFineGrainedPermissions(tt.keycloakApiClient(t))

			for _, idp := range objRealm.Spec.IdentityProviders {
				err := el.Serve(
