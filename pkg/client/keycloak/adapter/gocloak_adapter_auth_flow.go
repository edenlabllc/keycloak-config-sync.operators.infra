...
func (a GoCloakAdapter) adjustFlowExecutionPriority(
	...
	if childFlow.Priority != gocloak.PInt(flowExec.Priority) {
		flowExec.Priority = gocloak.IntP(childFlow.Priority)
		update = true
	}

	if childFlow.FlowV1 {
		flowExec.Priority = nil
	} else {
		if flowExec.Priority == nil {
			flowExec.Priority = gocloak.IntP(0)
		}
	}
