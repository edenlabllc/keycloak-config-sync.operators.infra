...
func (a GoCloakAdapter) SyncClientProtocolMapper(
	client *dto.Client, claimedMappers []gocloak.ProtocolMapperRepresentation, addOnly bool,
) error {
...
func (a GoCloakAdapter) GetClientProtocolMappers(client *dto.Client,
	clientID string,
) ([]gocloak.ProtocolMapperRepresentation, error) {
...
func (a GoCloakAdapter) findRoleMath(
	currentRoles []RealmRole,
	scopeRoles map[string]gocloak.Role,
) []gocloak.Role {
	matchRoles := make([]gocloak.Role, 0)

	for _, cRole := range currentRoles {
		if val, ok := scopeRoles[cRole.Name]; ok {
			matchRoles = append(matchRoles, val)
		}
	}
...
func (a GoCloakAdapter) filterRoles(roles map[string]gocloak.Role,
	isRoleClient bool,
) map[string]gocloak.Role {
	fRoles := make(map[string]gocloak.Role, 0)

	for cRoleName, cRole := range roles {
		if gocloak.PBool(cRole.ClientRole) == isRoleClient {
			fRoles[cRoleName] = cRole
		}
	}
