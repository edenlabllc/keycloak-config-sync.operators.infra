...
type RealmSettings struct {
	...
	WebAuthnPolicySettings    *WebAuthnPolicySettings
	Oauth2DeviceSettings      *Oauth2DeviceSettings
	OTPPolicySettings         *OTPPolicySettings
...
type WebAuthnPolicySettings struct {
	...
	PasswordlessRpEntityName                    string
	PasswordlessSignatureAlgorithms             []string
	PasswordlessRpID                            string
	PasswordlessAttestationConveyancePreference string
	PasswordlessAuthenticatorAttachment         string
	PasswordlessRequireResidentKey              string
	PasswordlessUserVerificationRequirement     string
	PasswordlessCreateTimeout                   int
	PasswordlessAvoidSameAuthenticatorRegister  bool
	PasswordlessAcceptableAaguids               []string
}
...
type ClientSessionSettings struct {
	IdleTimeout        int
	MaxLifespan        int
	OfflineIdleTimeout int
	OfflineMaxLifespan int
}
...
func setRealmSettings(realm *gocloak.RealmRepresentation, realmSettings *RealmSettings) {
	...
	if realmSettings.WebAuthnPolicySettings != nil {
		realm.WebAuthnPolicyRpEntityName = gocloak.StringP(realmSettings.WebAuthnPolicySettings.RpEntityName)
		realm.WebAuthnPolicyRpID = gocloak.StringP(realmSettings.WebAuthnPolicySettings.RpID)
		realm.WebAuthnPolicyAttestationConveyancePreference = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.AttestationConveyancePreference,
		)
		realm.WebAuthnPolicyAuthenticatorAttachment = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.AuthenticatorAttachment,
		)
		realm.WebAuthnPolicyRequireResidentKey = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.RequireResidentKey,
		)
		realm.WebAuthnPolicyUserVerificationRequirement = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.UserVerificationRequirement,
		)
		realm.WebAuthnPolicyCreateTimeout = gocloak.IntP(realmSettings.WebAuthnPolicySettings.CreateTimeout)
		realm.WebAuthnPolicyAvoidSameAuthenticatorRegister = gocloak.BoolP(
			realmSettings.WebAuthnPolicySettings.AvoidSameAuthenticatorRegister,
		)
		realm.WebAuthnPolicyPasswordlessRpEntityName = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.PasswordlessRpEntityName,
		)
		realm.WebAuthnPolicyPasswordlessRpID = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.PasswordlessRpID,
		)
		realm.WebAuthnPolicyPasswordlessAttestationConveyancePreference = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.PasswordlessAttestationConveyancePreference,
		)
		realm.WebAuthnPolicyPasswordlessAuthenticatorAttachment = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.PasswordlessAuthenticatorAttachment,
		)
		realm.WebAuthnPolicyPasswordlessRequireResidentKey = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.PasswordlessRequireResidentKey,
		)
		realm.WebAuthnPolicyPasswordlessUserVerificationRequirement = gocloak.StringP(
			realmSettings.WebAuthnPolicySettings.PasswordlessUserVerificationRequirement,
		)
		realm.WebAuthnPolicyPasswordlessCreateTimeout = gocloak.IntP(
			realmSettings.WebAuthnPolicySettings.PasswordlessCreateTimeout,
		)
		realm.WebAuthnPolicyPasswordlessAvoidSameAuthenticatorRegister = gocloak.BoolP(
			realmSettings.WebAuthnPolicySettings.PasswordlessAvoidSameAuthenticatorRegister,
		)

		if len(realmSettings.WebAuthnPolicySettings.SignatureAlgorithms) > 0 {
			realm.WebAuthnPolicySignatureAlgorithms = &realmSettings.WebAuthnPolicySettings.SignatureAlgorithms
		}

		if len(realmSettings.WebAuthnPolicySettings.AcceptableAaguids) > 0 {
			realm.WebAuthnPolicyAcceptableAaguids = &realmSettings.WebAuthnPolicySettings.AcceptableAaguids
		}

		if len(realmSettings.WebAuthnPolicySettings.PasswordlessSignatureAlgorithms) > 0 {
			realm.WebAuthnPolicyPasswordlessSignatureAlgorithms = &realmSettings.
				WebAuthnPolicySettings.PasswordlessSignatureAlgorithms
		}

		if len(realmSettings.WebAuthnPolicySettings.PasswordlessAcceptableAaguids) > 0 {
			realm.WebAuthnPolicyPasswordlessAcceptableAaguids = &realmSettings.
				WebAuthnPolicySettings.PasswordlessAcceptableAaguids
		}
	}

	if realmSettings.OTPPolicySettings != nil {
		realm.OtpPolicyAlgorithm = gocloak.StringP(realmSettings.OTPPolicySettings.Algorithm)
		realm.OtpPolicyDigits = gocloak.IntP(realmSettings.OTPPolicySettings.Digits)
		realm.OtpPolicyInitialCounter = gocloak.IntP(realmSettings.OTPPolicySettings.InitialCounter)
		realm.OtpPolicyLookAheadWindow = gocloak.IntP(realmSettings.OTPPolicySettings.LookAheadWindow)
		realm.OtpPolicyPeriod = gocloak.IntP(realmSettings.OTPPolicySettings.Period)
		realm.OtpPolicyType = gocloak.StringP(realmSettings.OTPPolicySettings.Type)

		if len(realmSettings.OTPPolicySettings.SupportedApplications) > 0 {
			realm.OtpSupportedApplications = &realmSettings.OTPPolicySettings.SupportedApplications
		}
	}
}
