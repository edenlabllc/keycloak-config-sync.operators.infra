...
type (
	...
	ServerSetupOption func(*ServerConfig)
)
			...
			http.MethodGet: {
				e.pathBuilder.AuthFlows(): func(w http.ResponseWriter, r *http.Request) {
					...
					_ = json.NewEncoder(w).Encode([]KeycloakAuthFlow{
						{Alias: flow.Alias, ID: existFlowID},
					...
					_ = json.NewEncoder(w).Encode([]FlowExecution{
						{
							...
							Priority:           gocloak.IntP(serverPriority),
							ID:                 "flow-exec-id",
					...
					_ = json.NewEncoder(w).Encode([]FlowExecution{
						{
							AuthenticationFlow: true,
							DisplayName:        "child-flow",
							Priority:           gocloak.IntP(1), // Same as flow priority
							ID:                 flowExecID,
