...
```sh
make docker-build docker-push IMG=<some-registry>/keycloak-config-sync:tag
```
...
```sh
make deploy IMG=<some-registry>/keycloak-config-sync:tag
```
...
```sh
make build-installer IMG=<some-registry>/keycloak-config-sync:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/keycloak-config-sync/<tag or branch>/dist/install.yaml
```
